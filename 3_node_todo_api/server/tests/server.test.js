...
const todos = [
  { _id: new ObjectID(), text: 'Dummie Todo 1' },
  {
    _id: new ObjectID(),
    text: 'Dummie Todo 2',
    completed: true,
...
describe('POST /todos', () => {
  ...
  it('should not create todo with invalid body data', done => {
    request(app).post('/post').send({}).expect(404).end((err, res) => {
      if (err) {
        return done(err);
      }
...
describe('PATCH /todos/:id', () => {
  it('should update the todo', done => {
    ...
    var updatedText = 'Some updated text';
  ...
  it('should clear completedAt when todo is not completed', done => {
    ...
    var updatedText = 'Some other updated text';
